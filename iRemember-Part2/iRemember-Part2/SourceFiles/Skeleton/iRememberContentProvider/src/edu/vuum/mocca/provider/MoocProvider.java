...
public class MoocProvider extends ContentProvider {
	...
	synchronized private Cursor query(final Uri uri, final String tableName,
		...
		Cursor cursor = mDB.query(tableName, projection, selection,
	...
	synchronized public Uri insert(Uri uri, ContentValues assignedValues) {
		...
		// ST:createUpsertForRelations:begin
			case STORY_ALL_ROWS : {
				...
				final long rowID = mDB.insert(MoocSchema.Story.TABLE_NAME,
						values);
				if (rowID < 0) {
					Log.d(LOG_TAG, "query()");
					return null;
			...
			case TAGS_ALL_ROWS : {
				...
				final long rowID = mDB.insert(MoocSchema.Tags.TABLE_NAME,
						values);
				if (rowID < 0) {
					Log.d(LOG_TAG, "query()");
					return null;
		...
		// ST:createUpdateForRelations:begin
			case STORY_SINGLE_ROW :
				whereClause = whereClause + MoocSchema.Story.Cols.ID + " = "
						+ uri.getLastPathSegment();
			case STORY_ALL_ROWS : {
				return updateAndNotify(uri, MoocSchema.Story.TABLE_NAME,
